 ...
 */
const HEARTBEAT_FREQUENCY_MS = 6000;
...
export class UserModel {
  ...
  constructor(
    ...
    this.name = new BehaviorSubject(
      this.isLocalUser() ? getAnimalName() : null
    );

    this.audioStream = new BehaviorSubject<MediaStream>(null);
    this.isMuted = this.isLocalUser()
      ? this.blindpadService.mediaService.getIsMuted()
      : new BehaviorSubject(false);
    this.voiceAnalyser = this.isLocalUser()
      ? this.blindpadService.mediaService.getLocalAnalyser()
      : new VoiceAnalyser(
          this.blindpadService.mediaService.getAudioContext(),
          this.blindpadService.zone
        );
  ...
  getColor(): BehaviorSubject<PaletteColor> {
    return this.color;
  }
  isLocalUser(): boolean {
    return this.userId === this.pad.getClientId();
  }
  ...
  isUnavailable(): boolean {
    return (
      this.isRemoteUser() &&
      Date.now() - this.lastMessageTime > PEER_UNAVAILABLE_MS
    );
  }
  isTimedOut(): boolean {
    return (
      this.isRemoteUser() && Date.now() - this.lastMessageTime > PEER_TIMEOUT_MS
    );
  }

  getAudioStream(): BehaviorSubject<MediaStream> {
    return this.audioStream;
  }
  getIsMuted() {
    return this.isMuted;
  }
  setMuted(muted = !this.getIsMuted().value) {
   ...
   */
  getMessagesIn(type: string): Observable<any> {
   ...
   */
  getMessagesOut(type: string): Observable<any> {
   ...
   */
  feedMessage(type: string, data: any) {
    this.messagesIn.next({ type: type, data: data });
  }
  ...
  start() {
    ...
    if (this.isLocalUser()) {
      this.pad.log('Connected user is: ', this.userId);
      return;
    }

    this.color.next(
      getColor(this.pad.getUsers() ? this.pad.getUsers().size : 0, true, true)
    );
  ...
  private isCaller(): boolean {
    return this.pad.getClientId() > this.userId;
  }
  ...
  private setupSubs() {
    this.broadcastSub = this.pad
      .getOutoingUserBroadcasts()
      .subscribe((msg) => this.messagesOut.next(msg));
    ...
    this.messagesOut
      .filter(
        (message) =>
          message.type !== ConnectionRequest.messageType &&
          message.type !== ConnectionResponse.messageType
      ) // these types are sent to the signaler instead of the webrtc channel
      .subscribe((message) => {
    ...
    this.getMessagesIn(ConnectionRequest.messageType)
      .take(1)
      .subscribe((request: ConnectionRequest) => {
        this.peerCxn
          .filter((cxn) => !!cxn)
          .take(1) // idea: filter on the cxn state instead of taking the first
          .subscribe((cxn) => {
            cxn.setRemoteDescription(JSON.parse(request.requestBlob));
            cxn.createAnswer().then(
              (desc) => {
                cxn.setLocalDescription(desc);
              },
              (error) => {
                console.error('Error creating answer to ', this.userId, error);
              }
            );
          });
      });
    this.getMessagesIn(ConnectionResponse.messageType)
      .take(1)
    ...
    this.getMessagesIn(UserStatusRequest.messageType).subscribe(
      (request: UserStatusRequest) => {
        this.sendHeartbeatResponse();
      }
    );
    this.getMessagesIn(UserStatusResponse.messageType).subscribe(
      (response: UserStatusResponse) => {
        ...
        if (response.update)
          this.feedMessage(PadUpdate.messageType, response.update);
      }
    );
  ...
  private setupRtc() {
    ...
    pc.onicecandidate = (candidate) => {
      ...
      if (this.isReceiver()) {
        ...
        this.messagesOut.next({
          type: ConnectionResponse.messageType,
          data: res,
        });
        ...
        this.messagesOut.next({
          type: ConnectionRequest.messageType,
          data: req,
        });
      }
    };
    ...
    const setupChannel = (channel: RTCDataChannel) => {
      ...
      channel.onclose = (event) =>
        console.error('channel [onclose]: ', this.userId, event);
      channel.onmessage = (event) => {
        this.blindpadService.zone.run(() => {
          // zonejs didnt think to monkeypatch webrtc
    ...
    if (this.isCaller()) {
      ...
      setupChannel(
        pc.createDataChannel(DATA_CHANNEL_NAME, DATA_CHANNEL_CONFIG)
      );
      pc.createOffer().then(
        (desc) => {
        ...
        (error) => {
          console.error('Error generating offer to ', this.userId, error);
        }
      );
