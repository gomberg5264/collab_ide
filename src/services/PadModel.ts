...
import {
    getSignalerURI,
    Message,
    PeersRequest, PeersUpdate, PadDisconnect,
...
export class PadModel {
    // public signaler: SocketIOClient.Socket;
    private useLog = true;
    ...
    start() {
        ...
        this.log('Backend Server is: ', signalerURI);
        ...
        this.signaler.on('connect', () => {
            this.log('Connected to backend server, Looking for peers!');
        ...
        this.signaler.on(PeersRequest.messageType, (data: PeersRequest) => {
            ...
            const disconnect = new PadDisconnect();
            disconnect.padId = this.padId;
            disconnect.srcId = this.clientId;
            disconnect.activePeers = Array.from(this.activePeers.values());
            disconnect.deadPeers = Array.from(this.deadPeers.values());
            this.signaler.emit(PadDisconnect.messageType, update);
        });
        ...
        this.signaler.on(PadDisconnect.messageType, (data: PadDisconnect) => {
            this.log(PadDisconnect.messageType, data);
            if (!this.isValidMessage(data)) return;
    ...
    private killUsersAndSignal(peerIds: Array<string>) {
        ...
        update.activePeers = Array.from(this.activePeers.values());
        update.deadPeers = Array.from(this.deadPeers.values());
        this.signaler.emit(PadUpdate.messageType, update);
        console.log(update);
    }

    private killPad(peerIds: Array<string>) {
        this.updateUsers([], peerIds);
        const update = new PeersUpdate();
        update.padId = this.padId;
        update.srcId = this.clientId;
